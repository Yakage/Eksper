...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="css/adminekspertakip.css">
      ...
      <main>
        <div class="post">
          <table class="rwd-table">
            <tr>
              <th>Eksper Adı</th>
              <th>Tamamladığı Değer Kaybı Sayısı</th>
              <th>Yanıtladığı İtiraz Sayısı</th>
            </tr>
            <?php 
            /*SELECT ad_soyad FROM tbl_kullanici WHERE id IN (select eksper_id from tbl_degerkaybibasvuru where eksper_id = 9)*/
            /*SELECT count(*) FROM tbl_degerkaybibasvuru WHERE eksper_id = 9*/  
            $kullanici_id =  $_SESSION["kullanici_id"];
            $eksperler = mysqli_query($baglanti, "SELECT id,ad_soyad FROM tbl_kullanici WHERE rol_id = 1");
            while ($eksperlerrow = mysqli_fetch_array($eksperler)) {
              $eksperid = $eksperlerrow['id'];
              $eksperad = $eksperlerrow['ad_soyad'];

              $kayitsayisi = mysqli_query($baglanti, "SELECT * FROM tbl_degerkaybibasvuru WHERE eksper_id = '$eksperid'");
              $kayitsayisirow = mysqli_num_rows($kayitsayisi);

              $itirazsayisi = mysqli_query($baglanti, "SELECT * FROM tbl_itiraz WHERE eksper_id = '$eksperid'");
              $itirazsayisirow = mysqli_num_rows($itirazsayisi);

              echo "<tr>
              <td data-th='Eksper Adı'>$eksperad</td>
              <td data-th='Tamamladığı Değer Kaybı Miktarı'>$kayitsayisirow</td>
              <td data-th='Tamamladığı Değer Kaybı Miktarı'>$itirazsayisirow</td>
              </tr>";
            }
            /*
